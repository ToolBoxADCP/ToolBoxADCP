%%Programme Modifié pour Mayotte
%%
function [nameu,fu,tidecon,xout]=t_tide(BolFig,fichierGlobal,fichierS,xin,varargin);
...
t=dt*([1:nobs]'-0*ceil(nobsu/2));  % Time vector for entire time series,
                                 % centered at series midpoint. 

if ~isempty(stime),
  centraltime=stime+0*floor(nobsu./2)./24.0*dt;
else
  centraltime=[];
end;
...
[nameu,fu,ju,namei,fi,jinf,jref]=constituents(ray/(dt*nobsu),constitnames,...
                                            shallownames,inf.iname,inf.irefname,centraltime);
 if (size(nameu,1)<10)
  [nameu,fu,ju,namei,fi,jinf,jref]=constituents(ray/(2*dt*nobsu),constitnames,...
                                             shallownames,inf.iname,inf.irefname,centraltime);
  %pause
 end
...
%xres ne doit pas etre très grand car tc*coef=xin !!!
...
jour=centraltime;
centraltime=jour;
% for centraltime=jour-100:1:jour+100;
