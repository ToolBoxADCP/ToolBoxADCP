...
%
%  si NFFT a trop de valeurs, on a un découpage en frequence d'une très grande 
%  résolution, mais du coup les pics deviennent moins significatifs 
%  => en général= NFFT = round(length(X)/n) ou n est tel que NFFT < 256?
%
%  si NFFT= length(X)/n, on découpe du coup l'intervalle en n morceaux, 
%  et le sepctre en fréquence est du coup une moyenne des n spectres
%
%   noverlap = 0 : permet d'éviter la fenêtre glissante 
%
%   hanning (NFFT): fonction permettant de lisser 
%                   (en donnant du poids aux valeurs du centre par ex.)
%
%  [dens,freq] = psd(X, length(X)/n, hanning
%
%  Fs = freq.d'échantillonage 
%
