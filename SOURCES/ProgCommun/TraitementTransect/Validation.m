...
Umesure=vitesse.u;Vmesure=vitesse.v;
ii=ceil(rand(N,1)*size(vitesse.u,1));
Uval=vitesse.u(ii,:);
Vval=vitesse.v(ii,:);
% dsurf=
% temps=
...
%%[u], [heure], [distance au mouillage], [prof] 
    B.u=Uval.u(:,i);
    B.v=Vval(:,i);
    B.prof=ProfondeurMouillage-dsurf(:,i)
...
%% Definition de la matrice A � l'aide de la vitesse au mouillage  
if(size(ii,1)~=0)
